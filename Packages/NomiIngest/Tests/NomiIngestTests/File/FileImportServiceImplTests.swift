...
final class FileImportServiceImplTests: XCTestCase {
  ...
  private actor RecordingIngester: DraftIngesting {
    ...
    /// Every draft handed over, in order, across every call.
    private(set) var drafts: [TransactionDraft] = []

    func ingest(_ batch: [TransactionDraft]) async throws -> IngestBatchResult {
      batchSizes.append(batch.count)
      drafts.append(contentsOf: batch)
      // One "created" per row, so the summary's arithmetic is checkable.
      return IngestBatchResult(created: batch.count, merged: 0, flagged: 0)
    }
  }
  ...
  // MARK: - W1-11 (M6): real date cells in an .xlsx

  /// A spreadsheet stores a date as a number, and only the cell's style says it
  /// is a date. Read without the style, every date arrived as its serial, no
  /// row mapped, and an HDFC export imported nothing.
  ///
  /// `date_cells.xlsx` has the HDFC preset's headers and three date cells in
  /// three formats: built-in 14, a custom `dd/mm/yyyy`, and a custom date and
  /// time whose serial carries a fraction. Generated by exceljs; the script is
  /// in the PR that added it.
  func testXLSXDateCellsParseEveryRowUnderTheHDFCPreset() async throws {
    let ingester = RecordingIngester()
    let service = FileImportServiceImpl(pipeline: ingester, now: Fixture.clock)

    let preview = try await service.inspect(fixture("date_cells.xlsx"))
    XCTAssertEqual(preview.detectedBankLabel, "HDFC")
    XCTAssertEqual(preview.parseableRowCount, 3, "every row, not zero")

    let mapping = try XCTUnwrap(preview.suggestedMapping)
    let summary = try await service.commit(fixture("date_cells.xlsx"), mapping: mapping, accountID: nil)
    XCTAssertEqual(summary.skipped, 0)

    // The days the sheet shows, not merely days that parse. The third cell's
    // 14:35 is dropped: a statement's date column is a date.
    let drafts = await ingester.drafts
    XCTAssertEqual(
      drafts.map(\.date),
      [Fixture.date("2026-04-01"), Fixture.date("2026-04-02"), Fixture.date("2026-04-06")]
    )
  ...
  /// Two genuine, identical purchases on one statement — two ₹20 teas — are two
  /// transactions, and each needs its own id or the pipeline keeps only one.
  /// Committing the same file again must produce the same ids, or a re-import
  /// is not a no-op.
  ///
  /// This holds on `main` as well: a row-index id is distinct within a file and
  /// stable across re-imports of the *same* file. What it pins is that changing
  /// how the id is built must not lose either property.
  func testIdenticalRowsWithNoReferenceGetDistinctIdsThatAreStableAcrossCommits() async throws {
    let ingester = RecordingIngester()
    let service = FileImportServiceImpl(pipeline: ingester, now: Fixture.clock)

    _ = try await service.commit(
      fixture("duplicate_rows_no_reference.csv"), mapping: noReferenceMapping, accountID: nil)
    let first = await ingester.drafts.map(\.sourceRef.externalID)
    _ = try await service.commit(
      fixture("duplicate_rows_no_reference.csv"), mapping: noReferenceMapping, accountID: nil)
    let all = await ingester.drafts.map(\.sourceRef.externalID)

    XCTAssertEqual(first.count, 2, "both rows are committed")
